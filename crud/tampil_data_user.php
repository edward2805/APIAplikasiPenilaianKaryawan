...
if($_SERVER['REQUEST_METHOD']=='GET'){
    ...
    tb_tugas.id_tugas, tb_tugas.status, tb_tugas.tipe, tb_tugas.start_tugas, tb_tugas.end_tugas, DATEDIFF (end_tugas, start_tugas) AS total_durasi 
    FROM tb_tugas 
    INNER JOIN user ON tb_tugas.user_id = user.id 
    INNER JOIN tenant ON tb_tugas.id_tenant = tenant.id_tenant
    WHERE user_id='$user_id' AND
    status LIKE 'open' ORDER BY tb_tugas.user_id DESC";
    ...
    if(mysqli_num_rows($sql)>0){
        ...
        while($rows=mysqli_fetch_array($sql)){
            ...
            $hasil["total_durasi"] = $rows["total_durasi"] ;
